...
- Added a shared SQLite migration framework (`schema_migrations`) used by branch-state and operation-log stores, expanded `/api/v1/status` with persistence details (store modes, DB path, schema versions), added `GET /api/v1/operations` filtering/paging (`status`, `type`, `limit`, `offset`), and introduced `mise` tasks for controller DB backup/restore (`db:backup`, `db:restore`).
- Branch and operation persistence now use separate SQLite files (`controller.db` and `operations.db`) to reduce lock contention; status now reports separate branch/operation DB paths, and backup/restore tasks now snapshot/restore both files together by directory.
